...
export async function getProgress(authUserId) {
  try {
    const { data: profile, error } = await supabase
      .from("user_profile")
      .select(
        "total_approaches, timer_runs, current_streak, longest_streak, last_approach_date, success_rate, past_successes, past_rejections"
      )
      .eq("id", authUserId)
      .single();
    ...
    return {
      ...
      pastRejections: profile.past_rejections || 0,
    };
  } catch (error) {
