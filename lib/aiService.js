...
export const generatePersonalizedCoaching = async (
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      max_tokens: 400, // Increased to allow for formatted responses with line breaks
    });
...
export const processPostActionReview = async (outcome, notes, userProfile) => {
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      temperature: 0.5, // Adjusted for warmer, friendlier tone
      max_tokens: 250, // Increased for formatted responses
    });
...
export const generateConversationStarter = async (
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      temperature: 0.5, // Adjusted for warmer, friendlier tone
      max_tokens: 200, // Increased for formatted responses
    });
...
export const generateMotivationQuote = async (userProfile) => {
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      temperature: 0.5, // Adjusted for warmer, friendlier tone
      max_tokens: 150, // Increased for formatted responses
    });
